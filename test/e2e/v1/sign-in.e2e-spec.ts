...
describe('/v1/sign-in (POST)', () => {
  ...
  describe('When an email that does not exists is provided', () => {
    const data = {
      email: faker.internet.email(),
      password: faker.internet.password(),
    };
  ...
  describe('When an invalid password is provided', () => {
    const data = {
      email: faker.internet.email(),
      password: faker.internet.password(),
    };
    ...
    beforeEach(async () => {
      ...
      const hashedPassword = crypto
        .pbkdf2Sync(faker.internet.password(), passwordSalt, 1000, 64, 'sha512')
        .toString('hex');
  ...
  describe('When valid credentials are provided', () => {
    ...
    beforeEach(async () => {
      const passwordSalt = crypto.randomBytes(16).toString('hex');
      const hashedPassword = crypto
        .pbkdf2Sync(data.password, passwordSalt, 1000, 64, 'sha512')
        .toString('hex');
